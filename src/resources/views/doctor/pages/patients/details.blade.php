...
@section('pageTitle'){{$authDoctor->name}} - Patient {{$patient->first_name}} {{$patient->last_name}} (#{{$patient->id}})@stop

@section('bodyClass', 'page-patients page-patients-details')
...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        <li><a href="{{route('doctor.patient.index')}}">Patients</a></li>
        <li class="active">{{$patient->getFullName()}} (#{{$patient->id}})</li>
    </ol>
@stop
            ...
            <div class="box box-success" id="box_medical_record_numbers">
                ...
                <div class="box-body">
                    <dl class="dl-horizontal">
                        ...
                        @if(in_array($clinic->id, $authDoctor->clinics->pluck('id')->all()))
                            ...
                            <tr data-id="{{$appointment->id}}">
                                ...
                                <td>
                                    @if($appointment->booker && $appointment->booker->id != $appointment->patient->id)
                                        <a href="{{ route('doctor.patient.details', $appointment->booker->id) }}">
                                            {{$appointment->booker->getFullname()}}
                                        </a>
                                    @endif
                                </td>
                                <td>
                                    <a href="{{route('appointment.show', $appointment->id)}}" data-toggle="tooltip" data-title="View appointent details">
                                        <i class="fa fa-info-circle"></i>
                                    </a>
        ...
        var modalEditTemplate = multiline(function(){/*!@preserve
            <form class="form" data-is-submitting="0">
                ...
                <div class="row">
                    <div class="col-xs-6">
                        <div class="form-group">
                            <input type="text" disabled name="phone_country_code" class="form-control" value="{{ $patient->phone_country_code }}"/>
                        </div>
                    </div>
                    <div class="col-xs-6">
                        <div class="form-group">
                            <input type="text" disabled name="phone_number" id="modal_edit_patient__form__input_phone_number" class="form-control" value="{{ $patient->phone_number }}"/>
                        </div>
                    </div>
                </div>
...
//                                    required: true
                            ...
                            submitHandler: function(form, event){
                                ...
                                var request = $.ajax({
                                    url: laroute.route("doctor.patient.update", {patient: {{$patient->id}}}),
                                    method: "POST",
                                    data: formData,
                                    dataType: "json"
                                });
                        ...
                        var buttons = {
                            'Set': {
                                ...
                                callback: function(event) {
                                    ...
                                    var request = $.ajax({
                                        url: laroute.route("doctor.patient.setGuardianship", {patient: {{$patient->id}}, guardian: targetId}),
                                        method: "POST",
                                        data: {
