...
/**
 * @namespace pageAdminBroadcastCreate
 * @param sandbox
...
module.exports = function(sandbox){
    ...
    /**
     * @memberOf pageAdminBroadcastCreate
     * @function render
     */
    _this.render = () => {
        _this.objects.$textAreaFullContent.wysihtml5();
        _this.objects.$selectCountry.chosen();
    };
    ...
    _this.bindEvents = () => {
        _this.objects.$groupAuthorIcon
            .dropzone({
                ...
                clickable: '#form_create_article__group_author_icon__button',
        ...
        _this.objects.$groupBanner
            .dropzone({
                ...
                paramName: 'file',
                previewsContainer: false,
                clickable: '#form_create_article__group_banner__button',
                previewTemplate: `<img data-dz-thumbnail/>`,
                url: laroute.route('admin.file.store'),
                autoProcessQueue: true,
                params: {
        ...
        _this.objects.$groupMediaThumbnail
            .dropzone({
                ...
                paramName: 'file',
                previewsContainer: false,
                clickable: '#form_create_article__group_media_thumbnail__button',
                previewTemplate: `<img data-dz-thumbnail/>`,
                url: laroute.route('admin.file.store'),
                autoProcessQueue: true,
                params: {
        ...
        _this.objects.$form.validate({
            ...
            submitHandler: (form, event) => {
                ...
                let request = $.ajax({
                    url: laroute.route('admin.broadcast.store'),
                    method: 'POST',
                    data: formData,
                    dataType: 'json',
                });
                ...
                request
                    .done((response) => {
                        let message = `Article has been created successfully.`,
                            id = _get(response, 'id')
                        ;

                        bootbox.alert(message, () => {
                            window.location.href = laroute.route('admin.broadcast.show', {id});
                        });
                    })
                    .fail((e, data) => {
                        let message = '';
    ...
    _this.init = ({data}) => {
        ...
        _this.objects = {};
        _this.objects.$form = $('#form_create_article');
        _this.objects.$textAreaFullContent = $('[name=full_content]');

        _this.objects.$groupBanner = $('#form_create_article__group_banner');
        _this.objects.$groupBanner_loadingOverlay = _this.objects.$groupBanner.find('.overlay');
        _this.objects.$banner = $('#form_create_article__group_banner__preview img');
        _this.objects.$inputBanner = $('[name=banner_url]');
        _this.objects.$selectCountry = $('#form_create_article__group_country');

        _this.objects.$groupAuthorIcon = $('#form_create_article__group_author_icon');
        _this.objects.$groupAuthorIcon_loadingOverlay = _this.objects.$groupAuthorIcon.find('.overlay');
        _this.objects.$authorIcon = $('#form_create_article__group_author_icon__preview img');
        _this.objects.$inputAuthorIcon = $('[name=author_icon_url]');

        _this.objects.$selectMediaType = $('[name="media[type]"]');
        _this.objects.$groupMediaVideo = $('#form_create_article__group_media_video');
        _this.objects.$groupMediaThumbnail = $('#form_create_article__group_media_thumbnail');
        _this.objects.$groupMediaThumbnail_loadingOverlay = _this.objects.$groupMediaThumbnail.find('.overlay');
        _this.objects.$mediaThumbnail = $('#form_create_article__group_media_thumbnail__preview img');
